...
class NetConfig:
    def __init__(self, vec_type='word2vec', vec_path='embedding/char_120wan_64_word2vec.vec', gpu_used=None, embedding_size=128, unit_num=256,
                 dropout_keep_rate=0.5, batch_size=32, seq_length=200, learning_rate=0.001, label_list=None,
                 iter_num=100, train_type='caner', feature_extractor='bilstm',
                 lambda_value=0.5, mask_flag='O', cluster_csv_list=None):
        """
        ...
        :param unit_num: The dimension of LSTM unit num/ CNN kernal/ Transformer dim before split heads
        ...
        :param feature_extractor: bilstm or idcnn or transformer
        """
        assert train_type in ['caner', 'common'], 'The train type must be "caner" or "common"'
        assert feature_extractor in ['bilstm', 'idcnn', 'transformer'], 'The feature extractor must be "bilstm" or "idcnn" or "transformer"'
        # if train_type == 'caner':
